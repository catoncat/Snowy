...
> 2026-06-01 修正：browser automation 的第一性原则以 `docs/browser-automation-first-principles.md` 为准。本文保留为 cutover 历史边界；其中 `page.query`、verify pipe、intervention 等条目不得被解释为继续扩张旧仓评分器、智能 locator、UID ranking、hidden recovery engine 或防御性兼容层的授权。UID-only `page.click` / `page.fill` 已从产品路径删除，默认主线改为 Browser Harness 原语。
...
| 层 | 状态 |
|----|------|
| descriptor 已声明 | `page.info/query/click_xy/type_text/press_key/scroll/screenshot`, `site.fetch_with_session`, `tabs.list/get_active/navigate` |
...
| 能力 | 新仓 Namespace | 理由 |
|------|---------------|------|
| 可见页面摘要 | `page.info` | 默认观察入口，替代 DOM snapshot 主线 |
| DOM 查询 / snapshot | `page.query` | 显式 debug DOM readback；不进入默认 Chat tool surface |
| 点击 | `page.click_xy` | 截图/坐标驱动的 Browser Harness 原语 |
| 填写 | `page.type_text` | 与 `page.click_xy` / `page.press_key` 组合完成表单输入 |
...
| ID | 状态 | 备注 |
|----|------|------|
| `page.info` / `page.click_xy` / `page.type_text` / `page.press_key` / `page.scroll` / `page.screenshot` | 已落地 | Browser Harness 主线；真实 dogfood 已覆盖 MDN/Maps |
| `page.query` | 非默认 debug readback | 不进入默认 Chat tool surface；删除条件见 dogfood todo |
| `page.click` / `page.fill` | 已删除 | UID-only 产品路径不再保留 |
...
|-------------|---------------|-------------|
| Browser Harness page primitives 真实实现 | `page.info/click_xy/type_text/press_key/scroll/screenshot` 走 MV3 runtime/page-hook 或 captureVisibleTab；`page.query` 仅显式 debug readback | core, site-runtime, mv3-shell |
...
- `ISSUE-154`：历史上补过 `page.query/click/fill` production path；2026-06-01 已纠偏为 `page.query` 非默认 debug readback，UID-only `page.click/page.fill` 删除
