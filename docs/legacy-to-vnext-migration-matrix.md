...
| Skill Studio / versions / lifecycle UI | old panel skills/plugins UI | future Skill Studio | keep-product-need | `not-started` | 生命周期模型、shared management consumer、`ISSUE-172` 的 executable skill invoke proof、`ISSUE-173` 的 setupPlan materialization proof、`ISSUE-174` 的 package manifest execution proof、`ISSUE-175` 的 package manifest action catalog discoverability 与 `ISSUE-176` 的 sidepanel catalog consumption 已有；完整 Skill Studio、版本管理与 authoring studio 仍没有；Soft Gate 1 已裁决为 cutover 后补 |
...
3. Skill Studio / lifecycle / versioning 产品面，以及 executable Skill 旧产品替代链路的更广覆盖；但下一步必须继续按旧产品替代 milestone 推进，不应把该项拆成孤立 UI/字段小票
