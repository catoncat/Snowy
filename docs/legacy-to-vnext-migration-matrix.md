...
| host shell / `host_bash` | bridge + old tool contracts | execution host substrate + `hosts.*` control plane | promote-and-simplify | `partial` | `hosts.*` control plane、`host.read/write/edit/exec` contract、default-host routing，以及默认 offscreen host 的显式 adapter error path 已落地并有测试；剩余 gap 是真实 local host adapter 与 remote host path 仍未实现 |
