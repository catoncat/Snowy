...
| host shell / `host_bash` | bridge + old tool contracts | execution host substrate + `hosts.*` control plane | promote-and-simplify | `partial` | `hosts.*` control plane、`host.read/write/edit/exec` contract、default-host routing 已落地；默认 offscreen local adapter 已实现 read/write/edit（ISSUE-038）；剩余 gap 是 exec 在 browser context 不可用（需 remote host）和 remote host path 仍未实现 |
| settings / runtime / skill management UI | panel stores + settings / skills / plugins surfaces | AI-native product control plane (`runtime.*` / `config.*` / `skills.*` / `hosts.*` / `audit.*`) | replace-with-control-plane | `partial` | bootstrap summary、`runtime.capture_diagnostics` / `runtime.clear_error`、最小本地 `hosts.*` control plane 与 `config.update` 已落地；剩余主缺口是 `skills.*` lifecycle control plane 与 `audit.*` |
...
| browser automation / background mode | `automation-mode.ts`, `stealth-tab.ts`, `background-failure-tracker.ts` | future `page.*` / `site.*` substrate | keep-core-ability | `review-gap` | cutover boundary 已裁决（见 `docs/browser-automation-cutover-boundary.md`）：Tier 1 (page.query/click/fill/press_key/screenshot + tabs.navigate/get_active + verify + intervention) 为 cutover 前必需；当前阶段沿用 site-runtime / MV3 独立路径，不要求先补 provider bridge；background mode / stealth tab 为 Tier 2 |
| screenshot / visual / download utilities | old builtin tools | future capability families | keep-by-capability | `review-gap` | 边界已裁决（见 `docs/screenshot-download-surface-boundary.md`）：`page.screenshot` 为 cutover 前最小视觉原语，落 `ISSUE-057`；`screenshot_with_highlight` 为 Tier 2 composite；download 延后到 product/workflow 层 |
