...
| host shell / `host_bash` | bridge + old tool contracts | execution host substrate + `hosts.*` control plane | promote-and-simplify | `partial` | `hosts.*` control plane、`host.read/write/edit/exec` contract、default-host routing 已落地；默认 offscreen local adapter 的真实边界现已锁定为 browser-only file adapter：继续承担读写编辑与 offscreen diagnostics，而 true exec parity 由 remote host path 承担（ISSUE-038 / ISSUE-073 / ISSUE-125 / ISSUE-126 / ISSUE-127 / ISSUE-134 / ISSUE-157）。execution host lane 仍保持 `partial`，因为 control-plane 仍缺 operation-aware host capability summaries / exec-capable default routing：`hosts.summary` 还无法一等表达 local host 是 file-only、remote host 才能 exec，operator 仍主要通过 `host.exec` 的 `operation_not_supported` 事后发现该边界（ISSUE-165） |
