...
| diagnostics / runtime debug / audit | debug snapshot + diagnostics HTTP | future observability layer | keep-and-rebuild | `partial` | 轻量 summary / `audit.tail` resource contract 与 runtime diagnostics action 已落地；但仍没有旧仓同等级 debug 面，也没有统一 app integration path |
