...
| Plugin Studio UI | `plugin-studio-main.ts` | future Skill Studio | replace | `partial` | 已有 shared management consumer、Skills catalog、package metadata/action catalog、lifecycle actions、version/rollback readiness read surface 与最小 rollback action；仍没有完整 authoring studio、rollback confirmation / version selection flow 或旧 plugin 生态批量迁移 |
...
3. Skill Studio / lifecycle / versioning 产品面与旧 plugin 生态更广 breadth；cutover-critical executable Skill 旧产品替代链路已由 `ISSUE-172` 到 `ISSUE-179` 收口到 install/persist/discover/manage/invoke/rollback/observe，不应再拆成孤立 UI/字段小票
