...
| Skill Studio / versions / lifecycle UI | old panel skills/plugins UI | future Skill Studio | keep-product-need | `not-started` | 生命周期模型、shared management consumer、`ISSUE-172` 的 executable skill invoke proof、`ISSUE-173` 的 setupPlan materialization proof、`ISSUE-174` 的 package manifest execution proof 与 `ISSUE-175` 的 package manifest action catalog discoverability 已有；完整产品 UI、版本管理与 studio 仍没有；Soft Gate 1 已裁决为 cutover 后补 |
