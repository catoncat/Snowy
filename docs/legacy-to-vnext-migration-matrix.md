...
| Plugin Studio UI | `plugin-studio-main.ts` | future Skill Studio | replace | `partial` | 已有 shared management consumer、Skills catalog、package metadata/action catalog、lifecycle actions 与 version/rollback readiness read surface；仍没有完整 authoring studio、交互式 rollback / version selection flow 或旧 plugin 生态批量迁移 |
...
| settings / runtime / skill management UI | panel stores + settings / skills / plugins surfaces | AI-native product control plane (`runtime.*` / `config.*` / `skills.*` / `hosts.*` / `audit.*`) | replace-with-control-plane | `partial` | descriptor-owned action projection、bootstrap summary、`runtime.summary/config.summary/skills.summary/hosts.summary/audit.tail` resource contract、`readAiSurfaceResource()` / MV3 `resource.read` 统一 lookup、`runtime.capture_diagnostics` / `runtime.clear_error`、最小本地 `hosts.*` / `config.update` / `skills.install/enable/disable/uninstall`、provider routing 的 `model.routing` shared control-plane、`ISSUE-093` 的 shared management consumer，以及 `ISSUE-172` / `ISSUE-173` / `ISSUE-174` / `ISSUE-175` / `ISSUE-176` / `ISSUE-177` 的 executable skill invoke + setupPlan materialization + package manifest execution + package action catalog discoverability + sidepanel catalog consumption + gate proof pack 纵向证明都已落地；`ISSUE-178` 继续把 shared version/rollback readiness 接到同一 Skills catalog。该 row 保持 `partial`，因为完整 Skill Studio / lifecycle product flow 仍未结束 |
...
| Skill Studio / versions / lifecycle UI | old panel skills/plugins UI | future Skill Studio | keep-product-need | `partial` | 生命周期模型、shared management consumer、`ISSUE-172` 的 executable skill invoke proof、`ISSUE-173` 的 setupPlan materialization proof、`ISSUE-174` 的 package manifest execution proof、`ISSUE-175` 的 package manifest action catalog discoverability、`ISSUE-176` 的 sidepanel catalog consumption 与 `ISSUE-178` 的 shared version/rollback readiness surface 已有；完整 authoring studio、交互式 version selection / rollback 和旧 plugin 生态迁移仍没有；Soft Gate 1 已裁决为 cutover 后补，但它现在是 full product completion lane |
...
3. Skill Studio / lifecycle / versioning 产品面与旧 plugin 生态更广 breadth；cutover-critical executable Skill 旧产品替代链路已由 `ISSUE-172` 到 `ISSUE-177` 收口，不应再拆成孤立 UI/字段小票
