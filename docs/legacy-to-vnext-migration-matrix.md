...
| skill package install / metadata / content injection | `skill-registry.ts`, `skill-content-resolver.ts`, `skill-create.ts` | `packages/skill-sdk`, BrowserVFS, Skill Studio shared surface | replace-and-simplify | `v0-shipped` | SDK 和 authoring 有基础；`ISSUE-172` 已证明代表性 executable skill 可经 shared MV3 runtime 完成 install/persist/enable/invoke/observe；`ISSUE-173` 已证明 `skills.install` setupPlan 可 materialize 到 `mem://skills/<skillId>/...`，重启后由 enabled skill 通过 `memfs.read` 读回；`ISSUE-174` 已证明重启后 runtime 可从 BrowserVFS package 的 `skill.json` 发现并注册 `handler.js`，再经 JS runner 执行；`ISSUE-175` 已证明 package manifest 的 actions / matches / requiresActiveTab / entry / version / kind / description / permissions / tags 可通过 shared `skills.summary` 与 `runtime.bootstrap` 被 AI/product surface 发现；`ISSUE-176` 已证明 sidepanel Skills catalog 会消费同一份 shared summary items 来 inspect package metadata 并触发现有 lifecycle actions；`ISSUE-177` 已把这条 cutover-critical proof 收口为 shipped-with-deferred-scope；`ISSUE-178` 已把 lifecycle/version engine 的 active version、snapshot root、rollback policy 和 rollback target 暴露到 shared `skills.summary` 与 sidepanel catalog；`ISSUE-179` 已把 rollback 从只读 readiness 变成 shared `skills.rollback` action，并证明 BrowserVFS rehydrate 后 summary/invoke/audit 都读回到回滚版本；`ISSUE-180` 已证明 Studio/package convention 数据可通过 shared `skills.install` create/update package-backed skill，更新时自动 snapshot 旧包并刷新 registry，再由 summary/invoke/rollback/audit 证明闭环；`ISSUE-181` 已把 package manifest 的 `eventSubscriptions` 投影到 `skills.summary` / `runtime.bootstrap`。剩余 version selection、diff/preview 与批量旧生态迁移是 post-cutover breadth |
| plugin runtime as extension model | `plugin-runtime.ts`, `plugin-sandbox.ts` | executable Skill + JS Runner | hard-cutover | `v0-shipped` | 方向已锁；`ISSUE-172` / `ISSUE-173` / `ISSUE-174` / `ISSUE-175` / `ISSUE-176` / `ISSUE-177` 已证明一条旧 plugin loop 可由 executable Skill 调用真实 `tabs.get_active` 能力、读取安装期 package 文件、自动加载 persisted package handler、暴露 package action catalog、进入可见 Skills catalog，并通过 `audit.tail` 留证；`ISSUE-181` 迁移 `send-success-global-message` 这类 hook-driven plugin：enabled package-backed Skill 可声明 `runtime.route.after` subscription，并由 shared MV3 `runtime.event.dispatch` 复用 `skills.invoke` + JS Runner 自动触发。完整旧 plugin 生态批量迁移仍是 post-cutover breadth，不回退到 Plugin product concept |
...
| tab / page interaction tools | old builtin page/tab tools | public namespaces `page.*`, `tabs.*` | replace-with-public-api | `partial` | 最小 public automation path 已纠偏为 Browser Harness（见 `docs/page-tabs-public-automation-path.md`）：Tier 1 = `page.info/click_xy/type_text/press_key/scroll/screenshot` + `tabs.get_active/navigate` + explicit debug/artifact/observability；`page.query` 仅为非默认 debug readback，UID-only `page.click/page.fill` 已删除；剩余 gap 已转到更广的 browser automation breadth 与 cutover 后范围 |
...
| hooks system / extension points | `hook-runner.ts`, plugin hooks | executable Skill setup/update/event subscription path | replace-and-simplify | `v0-shipped` | `packages/skill-sdk` 已提供 install-only declarative setup hook contract 与 `runSkillSetupHooks()` 计划 runner；shared MV3 runtime 已能把 install setupPlan 写入 BrowserVFS package root，在重启后从 package manifest 注册可执行 handler，并把 manifest action catalog 暴露到 shared `skills.summary`，sidepanel catalog 也会消费该 shared projection；`skills.rollback` 现已能把 trusted snapshot rehydrate 回 package root 并刷新 package registry；`ISSUE-180` 补齐 shared install/update path：更新已有 package 前自动 snapshot 旧包，更新后立刻刷新 registry并可 invoke/rollback；`ISSUE-181` 增加 runtime event subscription pilot，用 `runtime.event.dispatch` 触发 enabled package-backed Skill 的订阅 action。更丰富 phase、hook ordering、bulk hook matrix 与 version selection product UI 仍是后续 breadth |
...
| `ToolContract` 作为唯一真相源 | 不能统一投影到 SDK / runtime | `CapabilityDescriptor` |
...
2. provider policy hardening、bulk diagnostics / observability export。
