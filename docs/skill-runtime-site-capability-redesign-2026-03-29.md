...
> 日期：2026-03-29
> 目标：在不依赖 `@lifo-sh/core` 通用 shell 的前提下，保留 Browser Brain Loop 的浏览器侧大脑架构，重建可执行 Skill、`mem://` 文件系统、站点能力包装层，并收敛 `Skill / Plugin / Site Adapter` 的概念洪水。
>
...
4. 同一套能力既能给 Agent toolcall 用，也能给 Skill 用
...
- `Capability API`：唯一公共能力面，给 Skill / toolcall 共用
...
- capability provider registry 是当前最接近目标态的样板
- 相关文件：
...
- 对用户只保留一个核心概念：`Skill`
- 但 Skill 内部分为三类：`prompt`、`site`、`hybrid`
- 内部能力统一收敛成 `Capability API`，由 Skill / toolcall 共同消费
...
| `hybrid` | 同时含 prompt + site binding | 中 |
...
- 上层：LLM toolcall、Skill SDK、Studio 调试面板
...
如果需求需要组合多个现有能力，做 `hybrid skill`，只负责编排与教学，不重造底层能力。
...
ctx.capabilities.page.info({ maxElements: 20 });
ctx.capabilities.page.click_xy({ x: 420, y: 240 });
...
```ts
ctx.call("page.click_xy", { x: 420, y: 240 });
...
更合理的是让 `CapabilityDescriptor` 成为源头，再投影出 tool / SDK。

```ts
interface CapabilityDescriptor {
  id: string;                     // e.g. page.click_xy
...
但它们不够适合直接成为对 Skill / Studio 暴露的最终 public API。
...
新增更细粒度、AI-native 的公共命名空间，服务于 Skill SDK / Studio。
...
- `page.click_xy` / `page.type_text` / `page.press_key`
...
- `disabled`：保留安装但不可执行
- `archived`：从 active library 退出后的归档版本或回滚快照

产品面上的 `skills.uninstall` 应映射到 `archived`，而不是物理删除 skill 文件或版本历史。
...
> `Runtime` 负责真实执行。  
> 其中网站能力由 `Site Skill` 提供，建立在真实浏览器登录态、页面注入、CDP 与结构化动作之上。
...
8. 跨产品共享只保留 `SKILL.md` 级别的生态兼容，不把 site runtime 伪装成天然跨产品可执行
