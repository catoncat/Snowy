...
| skill SDK / authoring | `yellow` | typed facade 与文档已起步，且 `ISSUE-172` 已证明代表性 executable skill 可经 shared MV3 runtime 调用真实 `tabs.get_active` 能力；完整 authoring/studio 仍不足 |
| plugin -> executable skill migration | `yellow` | `ISSUE-172` 已证明一条旧 plugin loop 替代链路可由 executable skill 完成，但还不是完整旧 plugin 生态迁移状态 |
| Skill Studio / lifecycle product surface | `red` | 生命周期模型、shared management consumer 与 executable skill invoke proof 已有；完整产品 UI、版本管理和 authoring/studio 仍没有；Soft Gate 1 已裁决为 cutover 后补 |
