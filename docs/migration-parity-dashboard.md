...
| Skill Studio / lifecycle product surface | `yellow` | 生命周期模型、shared management consumer、executable skill invoke proof、setupPlan materialization proof、package manifest execution proof、package manifest action catalog discoverability、sidepanel catalog consumption，以及 `ISSUE-178` 的 shared version/rollback readiness surface 已有；完整 authoring studio、交互式 rollback / version selection flow、旧 plugin 生态批量迁移仍未完成；Soft Gate 1 仍是 cutover 后补，但已成为 full product completion lane |
