...
| skill SDK / authoring | `yellow` | typed facade、install-only setup hook contract、setupPlan forwarding、shared MV3 BrowserVFS materialization proof、package manifest runtime discovery 与 shared summary action catalog projection 已起步；`ISSUE-172` / `ISSUE-173` / `ISSUE-174` / `ISSUE-175` 证明代表性 executable skill 可经 shared MV3 runtime 调用真实 `tabs.get_active`、读取 setup 写入的 `mem://skills/...` package 文件、从安装后的 `skill.json` + `handler.js` 自动注册执行，并通过 `skills.summary` / `runtime.bootstrap` 暴露 manifest actions 与 site metadata；完整 authoring/studio 仍不足 |
| plugin -> executable skill migration | `yellow` | `ISSUE-172` / `ISSUE-173` / `ISSUE-174` / `ISSUE-175` 已证明一条旧 plugin loop 替代链路可由 executable skill 完成，并覆盖 install-time package content injection、persisted package handler execution 与 package action catalog discovery，但还不是完整旧 plugin 生态迁移状态 |
| Skill Studio / lifecycle product surface | `red` | 生命周期模型、shared management consumer、executable skill invoke proof、setupPlan materialization proof、package manifest execution proof 与 package manifest action catalog discoverability 已有；完整产品 UI、版本管理和 authoring/studio 仍没有；Soft Gate 1 已裁决为 cutover 后补 |
