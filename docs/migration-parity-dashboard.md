...
| skill SDK / authoring | `yellow` | typed facade、install-only setup hook contract、setupPlan forwarding、shared MV3 BrowserVFS materialization proof、package manifest runtime discovery、shared summary action catalog projection 与 sidepanel catalog consumption 已起步；`ISSUE-172` / `ISSUE-173` / `ISSUE-174` / `ISSUE-175` / `ISSUE-176` 证明代表性 executable skill 可经 shared MV3 runtime 调用真实 `tabs.get_active`、读取 setup 写入的 `mem://skills/...` package 文件、从安装后的 `skill.json` + `handler.js` 自动注册执行，通过 `skills.summary` / `runtime.bootstrap` 暴露 manifest actions/site metadata，并被可见 Skills catalog inspect/operate；完整 authoring/studio 仍不足 |
...
| Skill Studio / lifecycle product surface | `red` | 生命周期模型、shared management consumer、executable skill invoke proof、setupPlan materialization proof、package manifest execution proof、package manifest action catalog discoverability 与 sidepanel catalog consumption 已有；完整产品 UI、版本管理和 authoring/studio 仍没有；Soft Gate 1 已裁决为 cutover 后补 |
