...
- `ISSUE-172` / `ISSUE-173` / `ISSUE-174` 补齐的是 executable skill 的 shared runtime invoke、install setup materialization 与 package manifest execution 纵向证明，不改变完整 Skill Studio / lifecycle product UI 仍为 cutover 后补的裁决。
