...
当前最小已落地口径：`audit.tail` 已覆盖 `config.update`、`skills.discover/install/enable/disable/uninstall/rollback`、`hosts.connect/disconnect/set_default`。
...
`ISSUE-177` 对 cutover-critical 证据的结论是：旧产品替代主链已经从 review gap 收敛为一条可验证的 shipped-with-deferred-scope product loop。`ISSUE-178` 到 `ISSUE-181` 继续把 version/rollback、author/update 和 hook-driven plugin event subscription 接回同一条链路。当前 gate-by-gate 证据包见 `docs/level-2-cutover-acceptance-2026-05-27.md`。它证明的是一条代表性旧 Plugin / Skill 用户能力链已可由 vNext shared runtime 和 shared product surface 替代；它不是完整 Skill Studio、版本管理、旧 plugin 生态全迁移或外部切主线批准。
...
- Gate G: Agent/product consumer 可通过 `runtime.summary`、`config.summary`、`skills.summary`、`hosts.summary` 和 `runtime.bootstrap` 读取共享状态；sidepanel management 也消费同一 `skills.summary.items`，package actions/version/rollback readiness/event subscriptions 以及目录发现/导入都走 shared surface，不再维护 app-local package truth。
