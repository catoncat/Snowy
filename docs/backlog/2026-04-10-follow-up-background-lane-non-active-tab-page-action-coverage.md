...
- [ ] 至少一条测试通过显式 `automationTarget.lane = background` 跑通 `page.query -> page.fill -> page.click` 或等价多步 page flow
- [ ] 至少一条测试验证 `page.press_key` 或另一项 page action 在 non-active-tab background target 上可执行
- [ ] active-lane 默认约束保持不变；不得通过放宽默认 active-tab 匹配来“伪支持” background
- [ ] 覆盖落在共享 `site-runtime` / `mv3-shell` write scope 的正式测试中，而不是只停留在局部 harness 断言
