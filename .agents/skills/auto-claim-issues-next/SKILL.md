...
description: 为 browser-brain-loop-next 自动认领 backlog issue / slice。用于并行开发时挑选可开工 slice，遵守 depends_on，并把 write_scope 视为协调提示而不是自动阻塞。
---
...
10. `write_scope` 重叠不是默认 blocker；若不存在真实前后依赖，可在独立 worktree 中并行推进。
11. 使用 worktree 时仍要复用 canonical repo 的同一份 lease，不要把 worktree 当成新的 repo_id。
12. claim 成功后，默认立刻读取 issue / `acceptance_ref` 并进入实现，不要只回报 ticket 后等待批准。
13. 只有遇到真实 blocker（越权改动、缺失 acceptance、外部输入、并行冲突无法自行化解）才停下来问。
...
4. 对于“all live queue entries are already leased”：
   - 默认切到 `workflow:plan:preview` / `next-batch-planner` 的 read-only planning preview
   - 只有用户明确要求纯等待时，才只回报当前无 claim slot、等待 lease 释放
   - 不要自动重建 queue
